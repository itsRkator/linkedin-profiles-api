...
export const config = {
  ...
  database: {
    uri: process.env['MONGODB_URI'] ?? 'mongodb://localhost:27017/linkedin_profile_api',
  },
  ...
  linkedin: {
    ...
    requestTimeoutMs: Number.parseInt(process.env['LINKEDIN_TIMEOUT_MS'] ?? '25000', 10),
    retryMaxAttempts: Number.parseInt(process.env['LINKEDIN_RETRY_MAX'] ?? '0', 10),
    retryDelayMs: Number.parseInt(process.env['LINKEDIN_RETRY_DELAY_MS'] ?? '0', 10),
    minDelayMs: Number.parseInt(process.env['LINKEDIN_MIN_DELAY_MS'] ?? '8000', 10),
    maxJitterMs: Number.parseInt(process.env['LINKEDIN_MAX_JITTER_MS'] ?? '3000', 10),
  },

  cache: {
    ttlHours: Number.parseInt(process.env['PROFILE_CACHE_TTL_HOURS'] ?? '24', 10),
  },
