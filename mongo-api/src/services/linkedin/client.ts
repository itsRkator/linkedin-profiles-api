...
export class LinkedInBadResponseError extends Error {
  constructor(message: string) {
    super(message);
    this.name = 'LinkedInBadResponseError';
  }
}
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      try {
        ...
        const response = await undiciRequest(url, {
          ...
          headers,
          dispatcher: getAgent(),
          headersTimeout: config.linkedin.requestTimeoutMs,
          bodyTimeout: config.linkedin.requestTimeoutMs,
        });
        ...
        if (response.statusCode === 302) {
          ...
          if (setCookies.includes('li_at=delete me')) {
            throw new LinkedInAuthError(
              'LinkedIn session expired: credentials need to be refreshed',
            );
          }

          const location = getHeader(response.headers, 'location');
          if (location.startsWith('https://www.linkedin.com')) {
            logger.debug('Following benign LinkedIn 302 redirect', {
        ...
        if (response.statusCode === 403) {
          ...
          throw new LinkedInForbiddenError(
            'LinkedIn access denied: profile is private or forbidden',
          );
        }
        ...
        if (response.statusCode >= 500) {
          ...
          if (attempt < maxAttempts) {
            ...
            logger.warn(`LinkedIn transient error, retrying in ${delay}ms`, {
              attempt,
              status: response.statusCode,
            });
            await sleep(delay);
        ...
        if (!contentType.includes('json') && !contentType.includes('javascript')) {
          ...
          throw new LinkedInBadResponseError(
            `Unexpected content-type from LinkedIn: ${contentType}`,
          );
        }
        ...
        if (
          ...
          throw new LinkedInTimeoutError(
            `LinkedIn request timed out after ${config.linkedin.requestTimeoutMs}ms`,
          );
        }
        ...
        if (attempt < maxAttempts) {
          ...
          const delay = config.linkedin.retryDelayMs * 2 ** (attempt - 1);
          logger.warn(`LinkedIn error, retrying in ${delay}ms`, {
            attempt,
...
export function classifyFetchError(err: unknown): Omit<FetchResult, 'profile' | 'durationMs'> {
  if (err instanceof LinkedInAuthError)
    return { outcome: 'auth_failure', httpStatus: 401, errorCategory: 'auth_failure' };
  if (err instanceof LinkedInForbiddenError)
    return { outcome: 'forbidden', httpStatus: 403, errorCategory: 'forbidden' };
  if (err instanceof LinkedInNotFoundError)
    return { outcome: 'not_found', httpStatus: 404, errorCategory: 'not_found' };
  if (err instanceof LinkedInRateLimitError)
    return { outcome: 'rate_limited', httpStatus: 429, errorCategory: 'rate_limited' };
  if (err instanceof LinkedInTimeoutError)
    return { outcome: 'timeout', httpStatus: null, errorCategory: 'timeout' };
  if (err instanceof LinkedInBadResponseError)
    return { outcome: 'bad_response', httpStatus: null, errorCategory: 'bad_response' };
