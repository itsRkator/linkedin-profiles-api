// Usage: npx tsx scripts/capture-raw.ts <publicIdentifier>
  ...
  try {
    ...
    if (res.statusCode === 302) {
      ...
      if (loc.startsWith('https://www.linkedin.com')) {
        ...
        console.log(
          `✅ Saved (after redirect): /tmp/linkedin_raw_${identifier}.json (${body2.length} bytes)`,
        );
      }
      return;
    }
    ...
    try {
      ...
      if (parsed.included) {
        ...
        const types = [
          ...new Set((parsed.included as Record<string, unknown>[]).map((i) => i['$type'])),
        ].slice(0, 10);
